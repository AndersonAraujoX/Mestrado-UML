...
from src.core.operations import WorkOperation
...
class ErgotropyOptimizer:
    ...
    def __init__(
        ...
        Args:
            rho0 (qt.Qobj): Matriz de estado da bateria original (pré-otimização).
    ...
    def optimize(
        self, 
        work_op_class: type[WorkOperation], 
        pair_work: list, 
        ...
        initial_state = qt.Qobj(self.rho0, dims=dimension)
        ...
        while self.num_epochs > epoch and abs(loss - flag) > self.tol:
            ...
            for j in range(len(params)):
                ...
                work_op_plus = work_op_class(pair_work, params_plus)
                work_op_minus = work_op_class(pair_work, params_minus)
            ...
            work_op_f = work_op_class(pair_work, params)
            state_f = qt.Qobj(
