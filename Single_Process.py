...
def ergotropy_XY_troca(H_AB,params_ther,params_corr,pair_corr,pair_work, N):
...
def process_single_eb(eb_index, e_B, N, e_A, beta_A, beta_Bs, pair_corr, pair_work):
    ...
    Retorna os resultados Ws_XX e Ws_XY para esta fatia de e_B.
    """
    half = N // 2
    ...
    results_XY_for_eb = np.zeros(len(beta_Bs))
    ...
    num_params_XY = 2*len(pair_work) # Changed from 2*len(pair_work)
    results_params_XY_for_eb = np.zeros((len(beta_Bs), num_params_XY))
    ...
    for bb, beta_B in enumerate(beta_Bs):
        ...
        ws_XY,params_XY = ergotropy_XY(H_AB, [theta_A, theta_B], [theta, phi] * len(pair_corr), pair_corr, pair_work, N)

        results_XX_for_eb[bb] = ws_xx
        results_XY_for_eb[bb] = ws_XY

        results_params_XY_for_eb[bb,:] = params_XY
        results_params_XX_for_eb[bb,:] = params_xx

    return eb_index, results_XX_for_eb, results_XY_for_eb,results_params_XX_for_eb, results_params_XY_for_eb
    ...
    for ns, N in enumerate(tqdm(Ns, desc="Processando e_As")):
        ...
        with ProcessPoolExecutor() as executor:
            ...
            for future in tqdm(concurrent.futures.as_completed(futures), total=len(e_Bs), desc=f"Paralelizando e_Bs para e_A={e_A:.2f}"):
                ...
                Params_XY_N[eb_index,:,:] = results_params_XY_for_eb
